...
An ExpressRoute circuit consists of two links that connects the Customer/Provider edge to the Microsoft Enterprise Edge (MSEE) routers. Previously, as an example scenario, if your circuit bandwidth is 1 Gbps and you distribute your traffic evenly across both links, you could have achieved a maximum throughput of 2 Gbps (two times 1 Gbps).


Now, the combined traffic across both links will be restricted to the circuit's maximum throughput using rate limiting. Rate limiting restricts your throughput to the circuit's configured bandwidth if you exceed it on either link. The ExpressRoute circuit SLA is only guaranteed for the bandwidth that you configured. Continuing using the above example scenario, if you purchased a 1-Gbps circuit, your SLA is for a maximum throughput of 1 Gbps.
